...
fetchFriendsFast uid = do
  friends <- C.fql_query $
    "SELECT name, uid FROM user WHERE uid IN (SELECT uid2 FROM friend WHERE \
    \uid1 = " ++ show uid ++ ")"
...
fetchFriendStorage f_uid = do
  ...
  let
    assoc = concat $ map (\ f -> zip (BS.splitWith (== (BSI.c2w ' ')) .
      BS.pack . map (BSI.c2w) . map toLower $ name f) $ repeat f) friends
  return $ T.fromList $ assoc

getFriendsWithName :: (T.Trie FriendFast) -> String -> [(String, FriendFast)]
getFriendsWithName trie name = T.toListBy justNames subtrie where
